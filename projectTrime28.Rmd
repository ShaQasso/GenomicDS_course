...
Filtering the experimenatl desgin file to include only informative columns of metadata, specifically sex and group(Trim-KO or WT)
...
Just to note, the authors wrote in the paper's method section that they have filtered for genes that had a read counts average that is lower than 30 CPM reads. I think that their cut-off is too strict, and leaves out important genes (including Klf14, which is one of the main findings of the paper, thus I've decided to follow with the threshold of 3 CPM in at least 3 samples.
...
#### Creating a DeseQ object for RNAseq differential expression analysis

I combine the metadata and counts files into a DeseQ object.
...
ggplot(pcaData, aes(x = PC1, y = PC2, color = Group)) +
        ...
        #geom_text(label = rownames(pcaData), size = 4, nudge_x = -5, nudge_y = 0.2, show.legend = FALSE)+
...
```{r initiate deseq2 females only}
metadata_filtered_samples <- subset(metadata_filtered_samples, Sex != "male")
...
Gene ontolgy enrichment analysis revealed that those genes are related to reduced cell motility and development and increase in fatty acid metabolism - specifically processes related to the metabolism of very long chain fatty acids.
