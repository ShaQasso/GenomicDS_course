...
tgexp_non=merge(patient,tgexp_non,by="row.names")

# push sample ids back to the row names
rownames(tgexp_non)=tgexp_non[,1]
tgexp_non=tgexp_non[,-1]


intrain <- createDataPartition(y = tgexp_non[,1], p= 0.7)[[1]]

# seperate test and training sets
training_non <- tgexp_non[intrain,]
testing_non <- tgexp_non[-intrain,]

knnFit=knn3(x=training_non[,-1], # training set
            y=training_non[,1], # training set class labels
            k=5)
# predictions on the test set
trainPred=predict(knnFit,testing_non[,-1])

# predictions on the training set
trainPred=predict(knnFit,training_non[,-1],type="class")

# compare the predicted labels to real labels
# get different performance metrics
confusionMatrix(data=training_non[,1],reference=trainPred)

# predictions on the test set, return class labels
testPred=predict(knnFit,testing_non[,-1],type="class")

# compare the predicted labels to real labels
# get different performance metrics
confusionMatrix(data=testing_non[,1],reference=testPred)

```
*Comparing the accuracy measures, we can see that without transformations and scaling, our model was over-fitted to the training data, and dropped in accuracy trying to predict on the test set. *
...
*Tuned a grid to get the best mtry and min.node.size. *
...
# check prediction RMSE value
testPred <- predict(rfFit , testing)
RMSE(testPred,testing$Age)
