...
dds_res_all <- data.frame(res_Group) %>% mutate(threshold = ifelse((padj < 0.01 & log2FoldChange < 0), "Downregulated", 
...
p <- gost(query = list( "Upregulated" = rownames(signif_res_up), "Downregulated" = rownames(signif_res_dn)), 
          ...
          multi_query = TRUE,
          significant = TRUE,
          sources = c("KEGG", "GO:BP", "GO:CC", "WP", "REAC")
          )
...
ind <- c(
        sample(rownames(
                metadata_filtered_samples[metadata_filtered_samples$Sex == "male" & metadata_filtered_samples$Group == "Trim28KO", ]),3,FALSE),
        sample(rownames(
                metadata_filtered_samples[metadata_filtered_samples$Sex == "female" & metadata_filtered_samples$Group == "Trim28KO", ]),3,FALSE),
        sample(rownames(
                metadata_filtered_samples[metadata_filtered_samples$Sex == "male" & metadata_filtered_samples$Group == "WT", ]),3,FALSE),
        sample(rownames(
                metadata_filtered_samples[metadata_filtered_samples$Sex == "female" & metadata_filtered_samples$Group == "WT", ]),3,FALSE)
         )
